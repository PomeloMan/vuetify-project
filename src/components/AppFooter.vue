<template>
  <v-footer app height="40">
    ...
    <div class="text-caption text-disabled" style="position: absolute; right: 16px">
      ...
      —
      <span class="text-decoration-none on-surface"> Version {{ version }} </span>
    </div>
  </v-footer>
</template>
...
<script setup lang="ts">
  ...
  function getApplicationVersion() {
    get<string>('/version').then((data: string) => {
      version.value = data
    })
  }
  ...
  function toggleLocale() {
    ...
    i18nLocale.value = locale.value = locale.value === 'zh' ? 'en' : 'zh'
  }
